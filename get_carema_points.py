...
objp[:, :2] = 15*np.mgrid[0:8, 0:6].T.reshape(-1, 2)  # 将世界坐标系建在标定板上，所有点的Z坐标全部为0，所以只需要赋值x和y
...
print(obj_points[0][:, :2])
print(obj_points[0][:, :2].shape)
...
img = cv2.imread(os.path.join(root_path, image[0]))
h, w = img.shape[:2]
